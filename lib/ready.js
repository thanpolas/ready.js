...
{
  // check if structs Map is instanciated
  if (!ss.ready.db.allReady) {
    ss.ready.db.allReady = new goog.structs.Map();
  }
  
  var c = ss.ready.db.allReady.get(nameId);
  
  // if we found an instance return it
  if(c) {
    return c;
  }
...
ss.ready.C.prototype._runAllChecks = function(checkId)
{
  ...
  while(localfnCheckKey = fnChecks.next()) {
    ...
    if(fnCheck.checkId == checkId && goog.isFunction(fnCheck.fn)) {
      // found a listener for this check, check for delay
      if (0 == fnCheck.delay) {
        fnCheck.fn();
      } else {
        setTimeout(fnCheck.fn, fnCheck.delay);
      }
