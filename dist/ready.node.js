...
ss.ready.C.prototype.dispose = function() {
  this._dispose(this.fn);
  this._dispose(this.checks, true);
  this._dispose(this.fnCheck)
};
ss.ready.C.prototype._dispose = function(obj, opt_func) {
  for(var k in obj) {
    if(opt_func) {
      if(obj[k].hasFunc) {
        this._delete(this, obj[k].checkId)
      }
      this._delete(obj, k)
    }
  }
...
ss.ready.C.prototype.isDoneCheck = function(checkId) {
  return this.checks[checkId] && this.checks[checkId].done || false
};
...
ss.ready.C.prototype.addCheckListener = function(checkId, fn, opt_delay) {
  ...
  var obj = {checkId:checkId, fn:fn, delay:opt_delay || 0};
  var uid = ss.ready.uid();
...
ss.ready.C.prototype.removeCheckListener = function(uid) {
  for(var check in this.fnCheck) {
    if(this.fnCheck[check]) {
      this._delete(this.fnCheck[check], uid);
      break
    }
  }
};
ss.ready.C.prototype.addCheck = function(checkId) {
  if(!this.checks[checkId]) {
    this.checks[checkId] = {checkId:checkId, done:false, hasFunc:false};
    if(!this[checkId]) {
      this[checkId] = ss.ready.bind(function() {
        this.check(checkId)
      }, this);
      this.checks[checkId].hasFunc = true
    }
  }
};
...
ss.ready.C.prototype._runAll = function() {
  for(var ch in this.fnCheck) {
    ...
    this._delete(this.fnCheck, ch)
  }
  for(var fnid in this.fn) {
    if(ss.ready.isFunction(this.fn[fnid].fn)) {
      if(0 === this.fn[fnid].delay) {
        this.fn[fnid].fn()
      }else {
        this._runDelayed(this.fn[fnid].fn, this.fn[fnid].delay)
      }
    }
    this._delete(this.fn, fnid)
  }
};
  ...
  for(var ch in this.fnCheck[checkId]) {
    ...
    if(ss.ready.isFunction(chObj.fn)) {
      if(0 === chObj.delay) {
        ...
        this._runDelayed(chObj.fn, chObj.delay)
      }
    }
    this._delete(this.fnCheck[checkId], ch)
  }
  this._delete(this.fnCheck, checkId)
};
ss.ready.C.prototype._delete = function(obj, key) {
  return delete obj[key]
};
ss.ready.C.prototype._runDelayed = function(fn, delay) {
  setTimeout(fn, delay)
};
